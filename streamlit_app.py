...
# Google Drive 파일 ID
file_id = '1--aYbfIUAdrpbhvBo4luNF4BAqorBG4f'
...
def display_right_content(labels):
    ...
    for i, label in enumerate(labels[:3]):
        with cols[i]:
            st.image(f"https://i.ibb.co/CPLW6jX/guinea-pig-wallpaper-preview.jpg?text={label}", caption=f"이미지: {label}", use_column_width=True)

    # 2nd Row - YouTube Videos based on labels
    for i, label in enumerate(labels[:3]):
        with cols[i]:
            st.video("https://www.youtube.com/watch?v=3JZ_D3ELwOQ", start_time=0)
            st.caption(f"유튜브: {https://www.youtube.com/watch?v=9TSy2IyNJZc}")

    # 3rd Row - Text based on labels
    for i, label in enumerate(labels[:3]):
        with cols[i]:
            st.write(f"{label} 1교시.")
